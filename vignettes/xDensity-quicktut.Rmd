...
`xDensity` objects  can be constructed either from iid samples $\rv X n \iid f(x)$ or from a functional representation of $f(x)$ itself.  
<!-- are created from a non-parametric estimate of $f(x)$ obtained from a random sample $\rv X n \iid f(x)$.   -->
For example, consider the following `xDensity` representation based on a random sample from the log-Exponential distribution,
$$
...
<!-- First we create a generic function to compare a true distribution to its `xDensity` representation: -->
...
```{r, fig.width = 10, fig.height = 4, out.width = "97%", echo = -1}
#, fig.cap = "Accuracy test for `xDensity` approximation of `d/p/q/r` functions."}
# check that xDensity d/p/q/r functions match the true distribution,
# by plotting true PDF/CDF vs xDensity representation.
...
Package **`GaussCop`** currently implements three methods of constructing `xDensity` objects: two involve density estimation of a random sample $\rv X n \sim f(x)$, and the third, a functional approximation $\hat f(x)$, e.g., using a parametric model $f(x \mid \hat {\bm \theta})$, where $\hat{\bm \theta}$ is the parameter MLE.  
...
The `gc4XD` constructor implements a moment-based density approximation, which works best for unimodal distributions that are not too far from normal.

The estimator first applies a Box-Cox transformation to each observation $X_i$,
$$Z_i = \tx{BoxCox}(X_i), \qquad \tx{BoxCox}(x) = \begin{cases} \frac{((x + \alpha)^\lambda - 1)}{(\lambda C^{\lambda-1})} & \lambda \neq 0 \\ C \log(x + \alpha) & \lambda = 0, \end{cases}
$$
then fits $\rv Z n$ with a 4th order  [Gram-Charlier expansion](https://en.wikipedia.org/wiki/Edgeworth_series) [e.g., @draper.tierney72].  The example below illustrates the approximation for $M = 1000$ observations of $X \sim \chi^2_{(4)}$.

```{r, echo = -1, fig.width = 10, fig.height = 4, out.width = "97%"}
...
The `matrixXD` is used to convert functional density representations into the standardized `xDensity` format. Its input is a 2-column matrix with equally-spaced $x$-grid values in the first column and corresponding density values in the second. 
...
$$
2.  Since marginally $Z_i \sim \N(0,1)$, the Gaussian Copula approximates the true distirbution $g(\zz)$ by the best-match multivariate normal in the sense of minimizing the [Kullback-Leibler divergence](https://en.wikipedia.org/wiki/Kullback%E2%80%93Leibler_divergence),
$$
...
To evaluate the accuracy of the Gaussian Copula appromation, consider applying it to the 3-dimensional Cholesky decomposition of a $2\times 2$ unit [Wishart distribution](),
$$
\XX_{2\times 2} \sim \tx{Wishart}(\II, 1),
$$
where $\XX = \ZZ'\ZZ$ and $\ZZ$ is a $2\times 2$ matrix of iid $\N(0,1)$.  By construction, the Gaussian Copula perfectly captures the marginal distributions of $\tx{chol}(\XX)$.  Therefore, consider the error on the marginal distributions of the eigenvalues of $\XX$, induced by approximating the copula dependence between the Cholesky variables as multivariate normal.  In this case, the error appears to be relatively small:
```{r, fig.width = 10, fig.height = 4, out.width = "97%", fig.show="hold", cache = TRUE}
