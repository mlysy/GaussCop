...
Use of the `d/p/q/r` functions are illustrated with the following code.  First we create a generic function to compare a true distribution to its `xDens` representation:
```{r}
...
```
Now we can check the accuracy of the `xDens` approximation to the log-Exponential distribution:
```{r, fig.width = 10, fig.height = 4, out.width = "97%"}
dlexp <- function(x) exp(-exp(x) + x) # true PDF
plexp <- function(x) pexp(exp(x)) # true CDF
xdens_test(xDens, dtrue = dlexp, ptrue = plexp,
           dname = expression(log(X)%~%"Expo"*(1)),
           dlpos = "topleft", plpos = "topleft")
