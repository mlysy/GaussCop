...
```
The `d/p/q/r` functions for `xDensity` objects are invoked by e.g., `dXD(x, xDens = xDens, log = TRUE)` as seen in the [Appendix](#apptest) **R** code for the test function `xdens_test`.  That is, consider the following graphical check of an `xDensity` approximation to the log-Exponential distribution $\log(X) \sim \tx{Expo}(1)$:
...
1.  The data are first normalized by applying a Box-Cox transformation to each observation $X_i$,
...
To evaluate the accuracy of the Gaussian Copula appromation, consider applying it to the $p(p+1)/2$-dimensional Cholesky decomposition of a $p\times p$ unit [Wishart distribution](),
$$
\XX_{p\times p} \sim \tx{Wishart}(\II, 1),
$$
where $\XX = \ZZ'\ZZ$ and $\ZZ$ is a $p\times p$ matrix of iid $\N(0,1)$.  By construction, the Gaussian Copula perfectly captures the marginal distributions of $\tx{chol}(\XX)$.  Therefore, consider the error on the marginal distributions of the eigenvalues of $\XX$, induced by approximating the copula dependence between the Cholesky variables as multivariate normal.  In the example below with $p = 4$, the error appears to be relatively small:
```{r eigen, cache = TRUE}
...
```
```{r, fig.width = 10, fig.height = 2.5, out.width = "97%", fig.show = "hold"}
